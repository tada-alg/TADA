...
def get_indices_training_and_test(labels, most_freq_class):
...
def make_test_dataset(X_test, meta_fp, class_label, samples_orig_tr, labels_orig_tr, samples_orig_ts, y_test, samples_augm, logger_ins, testing_ratio, direction):
	...
	freq_dct_tr, most_freq_class_tr, least_freq_class_tr = make_freq_dict(labels_orig_tr)

	mapping_tr = get_indices_training_and_test(labels_orig_tr, most_freq_class_tr)
	mapping_ts = get_indices_training_and_test(y_test, most_freq_class_tr)
	...
	print("The most prev in training is", len(samples_orig_ts[mapping_ts['most']]), "in test data and least prev class in training is", len(samples_orig_ts[mapping_ts['least']]), "in test data", direction)
	...
	freq_dct_ts, _, _ = make_freq_dict(y_test)
...
def get_classifier_obj(method, n_jobs, seed_num, logger_ins, class_weight):
	if method == "rf":
...
def balance_with_subsample(X, y, samples_orig, samples_augm, augm_levels, down_sample, logger_ins, most_freq_class, least_freq_class):

	if augm_levels >=0:
		samples_augm_unique, samples_count = np.unique(samples_augm, return_counts=True)
		freq_mapping = dict()
		for i in range(len(samples_augm_unique)):
			freq_mapping[samples_augm_unique[i]] = samples_count[i]

		augm_to_increase = np.isin(samples_orig, samples_augm_unique)

		S_orig = samples_orig[augm_to_increase]
		S_counts = list()
		for i in range(len(S_orig)):
			S_counts.append(freq_mapping[S_orig[i]])

		X_subsampled_train = np.row_stack([X, np.repeat(X[augm_to_increase], S_counts,axis=0)])
		y_subsampled_train = np.row_stack([y.reshape((-1,1)), np.repeat(y[augm_to_increase], S_counts,axis=0).reshape((-1,1))])

		freq_dct, _, _ = make_freq_dict(y_subsampled_train)
		...
		most_freq_idx = np.asarray(sorted(np.where(np.isin(y, most_freq_class[1]))[0].ravel()))
		least_freq_idx = np.asarray(sorted(np.where(np.isin(y, least_freq_class[1]))[0].ravel()))
		samples_to_select = np.random.choice(most_freq_idx, size=least_freq_class[0], replace=False)
		augm_to_increase = np.row_stack((samples_to_select.reshape((-1,1)), least_freq_idx.reshape((-1,1)))).ravel()
...
def upsample_data(X, y, augm_levels, logger_ins):
	freq_dct, most_freq_class, least_freq_class = make_freq_dict(y)
	...
	freq_dct, _, _ = make_freq_dict(X_new)
...
def balance_smote(X, y, logger_ins, freq_dct, method, seed_num, n_cluster=5):
	if method == "SMOTE":
		...
		imbl = SMOTE(sampling_strategy=freq_dct, random_state=seed_num, k_neighbors=n_cluster)
	else:
		imbl = ADASYN(sampling_strategy=freq_dct, random_state=seed_num, n_neighbors=n_cluster)
	logger_ins.info("The sampling method is", method, "using", n_cluster, "as the number of clusters")
...
if __name__ == "__main__":
	...
	parser.add_option("-u", dest="use_augm", default=0, type=int,
					  help="Wheather to use augmented data or not")

	parser.add_option("-w", dest="class_weight", default='original',
					  help="Wheather to do do class weighting or not (for random forest). The default is None, "
						   "and other options are original, balanced_subsample, and balanced")
	parser.add_option("--num_clusters", dest="n_clusters", default=5, type=int,
					  help="The number of clusters to be used with SMOTE")
	...
	class_weight = options.class_weight if options.class_weight != 'original' else None
	...
	logger_ins.info("The seed number is", seed_num)
	...
	t0 = time()
	freq_dct, most_freq_class, least_freq_class  = make_freq_dict(labels_orig_tr)
	...
	results['labels_augm'] = labels_augm
	results['labels_orig_tr'] = labels_orig_tr
	freq_augm = Counter(y_train)
	_, most_freq_class, least_freq_class = make_freq_dict(labels_orig_tr)

	X_subsampled_train, y_subsampled_train = balance_with_subsample(X_orig_tr, labels_orig_tr, samples_orig_tr, samples_augm, augm_levels, 0, logger_ins, most_freq_class, least_freq_class)
	if X_subsampled_train is not None:
		X_test_upsample_predict, X_test_upsample_proba = do_clssification(clf, X_subsampled_train, y_subsampled_train, X_test, y_test, 'augmented with subsampling ', output_label, logger_ins)
		del X_subsampled_train
		results['X_test_upsample_predict'] = X_test_upsample_predict
		results['X_test_upsample_proba'] = X_test_upsample_proba

	X_SMOTE_train, y_SMOTE_train = balance_smote(X_orig_tr, labels_orig_tr, logger_ins, freq_augm, 'SMOTE', seed_num, n_cluster=n_clusters)
	X_ADASYN_train, y_ADASYN_train = balance_smote(X_orig_tr, labels_orig_tr, logger_ins, freq_augm, 'ADASYN', seed_num, n_cluster=n_clusters)
	if X_SMOTE_train is not None:
		X_test_SMOTE_predict, X_test_SMOTE_proba = do_clssification(clf, X_SMOTE_train, y_SMOTE_train, X_test,
																	y_test, 'augmented with SMOTE', output_label,
																	logger_ins)
		del X_SMOTE_train
		results['X_test_SMOTE_proba'] = X_test_SMOTE_proba
		results['X_test_SMOTE_predict'] = X_test_SMOTE_predict

		X_test_ADASYN_predict, X_test_ADASYN_proba = do_clssification(clf, X_ADASYN_train, y_ADASYN_train, X_test,
																	  y_test, 'augmented with SMOTE', output_label,
																	  logger_ins)
		del X_ADASYN_train
		results['X_test_ADASYN_proba'] = X_test_ADASYN_proba
		results['X_test_ADASYN_predict'] = X_test_ADASYN_predict

	if experiment == 'unbalance':
		X_downsampled_train, y_downsampled_train = balance_with_subsample(X_orig_tr, labels_orig_tr, samples_orig_tr, samples_augm, -1, 1, logger_ins, most_freq_class, least_freq_class)
		if X_downsampled_train is not None:
			X_test_downsample_predict, X_test_downsample_proba = do_clssification(clf, X_downsampled_train, y_downsampled_train, X_test, y_test, 'augmented with downsampling', output_label, logger_ins)
			del X_downsampled_train
			results['X_test_downsample_proba'] = X_test_downsample_proba
			results['X_test_downsample_predict'] = X_test_downsample_predict
